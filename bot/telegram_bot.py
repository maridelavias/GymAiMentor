...
MUSCLE_GROUPS_KEYBOARD = ReplyKeyboardMarkup(
    [
        ...
        ["🔙 Упор на спину", "🏋️ Упор на грудь"],
        ["💪 Упор на плечи и руки", "🦾 Упор только на плечи"],
        ["🤜 Упор только на руки", "🧘 Кор и пресс"],
        ["🔄 Функциональные", "🎲 Сбалансированная программа"],
        ["◀️ Назад в меню"],
    ],
    resize_keyboard=True,
    one_time_keyboard=True,
    ...
    if text in variation_map:
        ...
        try:
            ...
            plan = await agent.get_program(
                variation,
                focus_group_override=muscle_group if (muscle_group and muscle_group != "сбалансированно") else None,
            )
